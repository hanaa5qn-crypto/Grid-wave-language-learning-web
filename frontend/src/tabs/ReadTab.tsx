                      ...
                      <div className="flex gap-1 mb-3">
                        {LIB_LEVELS.map(lv => (
                          <button key={lv} onClick={() => setLibReadLevel(lv)}
                            className={gold || aurora ? `flex-1 py-1.5 rounded-lg border-2 border-on-background text-xs font-bold cursor-pointer transition-colors ${libReadLevel === lv ? 'bg-secondary text-white' : 'bg-surface-container text-on-surface-variant'}` : `flex-1 py-1.5 rounded-lg border-2 border-ink-line text-xs font-bold cursor-pointer transition-colors ${libReadLevel === lv ? 'bg-paper text-ink' : 'bg-ink-raise text-paper-2'}`}>
                            {lv === 'all' ? 'Бүгд' : lv}
                          </button>
                        ))}
                      </div>
                        ...
                        {filtered.map(r => {
                          ...
                          return (
                            <button key={r.id} onClick={() => openReadItem(r)}
                              className={gold || aurora ? `text-left p-2.5 rounded-lg border-2 border-on-background cursor-pointer transition-colors ${r.id === libReadId ? 'bg-secondary-container' : 'bg-surface-container hover:bg-surface-container-high'}` : `text-left p-2.5 rounded-lg border-2 border-ink-line cursor-pointer transition-colors ${r.id === libReadId ? 'bg-ink-raise' : 'bg-ink-raise hover:bg-ink-2'}`}>
                          ...
                          <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="flex gap-2">
                              ...
                              <button onClick={() => setLibReadTrans(v => !v)}
                                className={gold || aurora ? `px-3 py-1 border-2 border-on-background rounded-full font-bold text-xs block-shadow cursor-pointer hover:scale-105 transition-transform flex items-center gap-1 ${libReadTrans ? 'bg-secondary text-white' : 'bg-surface-container text-on-surface'}` : `px-3 py-1 border-2 border-ink-line rounded-full font-bold text-xs block-shadow cursor-pointer hover:scale-105 transition-transform flex items-center gap-1 ${libReadTrans ? 'bg-paper text-ink' : 'bg-ink-raise text-paper'}`}>
                                <Languages className="w-4 h-4" /> {libReadTrans ? 'Нуух' : 'Орчуулга'}
                              </button>
                            </div>
                          </div>

                          <h2 className={gold || aurora ? "text-2xl md:text-3xl font-extrabold text-on-surface mb-1 tracking-tight" : "text-2xl md:text-3xl font-extrabold text-paper mb-1 tracking-tight"}>{item.title}</h2>
                          <p className={gold || aurora ? "text-sm text-on-surface-variant mb-5" : "text-sm text-paper-2 mb-5"}>{item.titleMn}</p>

                          <p className={gold || aurora ? "text-lg leading-relaxed text-on-surface whitespace-pre-line font-medium" : "text-lg leading-relaxed text-paper whitespace-pre-line font-medium"}>{renderRichGerman(item.text)}</p>
                          {libReadTrans && (
                            <p className={gold || aurora ? "text-sm leading-relaxed text-on-surface-variant whitespace-pre-line mt-4 pt-4 border-t border-outline-variant/50 italic" : "text-sm leading-relaxed text-paper-2 whitespace-pre-line mt-4 pt-4 border-t border-ink-line/50 italic"}>{item.translation}</p>
                          )}
